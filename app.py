...
# Filtro para formatar data (ex: '2023-07-15' -> '15/07/2023')
...
def dashboard():
    ...
    try:
        ...
        for i in range(11, -1, -1): 
            m = (hoje.replace(day=1) - timedelta(days=1)).replace(day=1) 
            data_ref = (hoje.replace(day=1) - timedelta(days=0)) - timedelta(days=0) 
            
            ano = (hoje.year * 12 + hoje.month - 1 - i) // 12
            mes = (hoje.year * 12 + hoje.month - 1 - i) % 12 + 1
        ...
        with sqlite3.connect('database.db') as conn:
            ...
            # receitas por mês (strftime pega 'YYYY-MM' do campo data)
            cursor.execute("""
                SELECT strftime('%Y-%m', data) as ym, IFNULL(SUM(valor),0)
                FROM transacoes
                WHERE usuario_id = ? AND tipo = 'receita'
                GROUP BY ym
            """, (usuario_id,))
            receitas_por_mes = dict(cursor.fetchall())  # mapping ym -> sum
        ...
        # monta as listas na ordem dos últimos 12 meses
...
def exportar_pdf():
    ...
    # Conecta ao banco e busca as transações
    conn = sqlite3.connect('database.db')
    cursor = conn.cursor()
